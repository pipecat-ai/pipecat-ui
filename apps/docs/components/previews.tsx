...
export function ConnectButtonPreview() {
  ...
  return (
    <PreviewShell
      controls={
        <>
          <ControlSection title="Connection">
            <SelectControl
              label="state"
              value={state}
    ...
    <PreviewShell
      ...
      controls={
        <>
          ...
          <ControlSection title="Availability">
            <BooleanControl
              label="disabled"
              checked={disabled}
    ...
    <PreviewShell
      ...
      controls={
        <>
          ...
          <ControlSection title="Peak markers">
            <BooleanControl
              label="noPeaks"
              checked={noPeaks}
...
// Message timestamps use the browser's locale and timezone.
const ConversationView = dynamic(
  () =>
    import("@/components/pipecat/conversation").then(
      (module) => module.ConversationView,
    ),
  { ssr: false },
);

const CONVERSATION_START = Date.parse("2026-01-01T10:00:00Z");
const MESSAGES = [
  {
    role: "user",
    createdAt: new Date(CONVERSATION_START).toISOString(),
    parts: [{ text: "Hey! What's the weather like in Amsterdam?" }],
  },
  {
    role: "function_call",
    createdAt: new Date(CONVERSATION_START + 5_000).toISOString(),
    functionCall: {
  ...
  {
    role: "assistant",
    createdAt: new Date(CONVERSATION_START + 10_000).toISOString(),
    parts: [
      {
        text: {
  ...
  {
    role: "assistant",
    createdAt: new Date(CONVERSATION_START + 55_000).toISOString(),
    parts: [],
  },
    ...
    <PreviewShell
      controls={
        <>
          ...
          <ControlSection title="Send button">
            <TextControl
              label="buttonContent"
              value={buttonLabel}
