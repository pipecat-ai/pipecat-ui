...
export interface ReactShaderToyProps {
  ...
  /** Textures are resized to power-of-two dimensions for mipmaps and repeat wrapping. */
  textures?: TextureParams[];

  /** Custom uniform values, read each frame. */
  uniforms?: Uniforms;

  /** RGBA canvas clear color. */
  clearColor?: Vector4;
...
export function ReactShaderToy({
  ...
  const drawFrameRef = useRef<FrameRequestCallback | null>(null);
  ...
  // Cache uniform locations after linking, outside the frame loop.
  ...
  const onResize = () => {
    ...
    if (uniformsRef.current.iResolution?.isNeeded && shaderProgramRef.current) {
      gl.uniform2fv(uniformLocationsRef.current[UNIFORM_RESOLUTION] ?? null, [
        gl.canvas.width,
        gl.canvas.height,
      ]);
    }
  ...
  const initShaders = (fragmentShader: string, vertexShader: string) => {
    ...
    // Null locations are valid for uniforms optimized out by the compiler.
    uniformLocationsRef.current = {};
    for (const name of Object.keys(uniformsRef.current)) {
      if (uniformsRef.current[name]?.isNeeded) {
        uniformLocationsRef.current[name] = gl.getUniformLocation(
          shaderProgramRef.current,
          name,
        );
      }
  ...
  const setUniforms = (timestamp: number) => {
    ...
    if (propUniforms) {
      for (const name of Object.keys(propUniforms)) {
        ...
        if (uniformsRef.current[name]?.isNeeded) {
          const customUniformLocation = locations[name];
    ...
    if (uniformsRef.current.iDate?.isNeeded) {
      ...
      gl.uniform4fv(locations[UNIFORM_DATE] ?? null, [year, month, day, time]);
    }
    if (uniformsRef.current.iFrame?.isNeeded) {
      const frame = uniformsRef.current.iFrame.value as number;
      gl.uniform1i(locations[UNIFORM_FRAME] ?? null, frame);
    ...
    if (texturesArrRef.current.length > 0) {
      for (let index = 0; index < texturesArrRef.current.length; index++) {
        ...
        if (uniformsRef.current[`iChannel${index}`]?.isNeeded) {
          const iChannel = locations[`${UNIFORM_CHANNEL}${index}`] ?? null;
  ...
  useEffect(() => {
    ...
    if (animateWhenNotVisible) {
      ...
      if (drawFrameRef.current) {
        ...
        animFrameIdRef.current = requestAnimationFrame(drawFrameRef.current);
      }
    }
  }, [animateWhenNotVisible]);

  // Resume only one frame loop after visibility changes.
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        for (const entry of entries) {
          ...
          if (entry.isIntersecting && drawFrameRef.current) {
            cancelAnimationFrame(animFrameIdRef.current ?? 0);
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      if (gl && canvasRef.current) {
        ...
        processTextures();
        if (!initShaders(preProcessFragment(fs || BASIC_FS), vs || BASIC_VS))
          return;
        initBuffers();
        drawFrameRef.current = drawScene;
        cancelAnimationFrame(animFrameIdRef.current ?? 0);
        animFrameIdRef.current = requestAnimationFrame(drawScene);
        addEventListeners();
        onResize();
    ...
    // Shader configuration is fixed per mount; uniforms update through refs.
