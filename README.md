...
| `--agent`               | Agent conversation role color.          |
| `--client`              | User conversation role color.           |
...
| ---------------------- | ---------------- | ------------------------------ |
| `pipecat-ui-docs`      | `apps/docs`      | Next.js docs and `/r` registry |
| `pipecat-ui-storybook` | `apps/storybook` | `storybook-static`             |

Use `main` as each project's production branch, select Node.js 22.x, and enable
access to files outside its root directory. Both apps include `vercel.json`; CI builds and validates them,
...
GitHub release if missing. Set the GitHub repository variable `VERCEL_DOCS_PROJECT`
to `pipecat-ui-docs` (or your docs project's name). Enable Vercel's repository
